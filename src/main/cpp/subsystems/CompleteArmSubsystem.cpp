        ...
        [this, type] (){
            switch (type) {
                case MovementType::None:
                    // * Current cone color
                    this->m_leds->setOn();
                    this->m_leds->setColor(this->m_leds->getCurrentColor());
                    this->m_leds->setPattern(LEDControllerSubsystem::PatternType::SetAll, true);
                    break;
                case MovementType::HomeAll:
                    this->m_leds->setOn();
                    // * Light blue
                    this->m_leds->setColor(30, 30, 200);
                    this->m_leds->setPattern(LEDControllerSubsystem::PatternType::Blink);
                    break;
...
frc2::CommandPtr CompleteArmSubsystem::AutoIntake() {
    return TravelMode()
        ...
        .Until(
            [this] (){
                return this->m_lidar->GetDistance() <= 400;
            }
        )
        // Stop moving
...
frc2::CommandPtr CompleteArmSubsystem::TravelMode() {
    ...
    return SetMovementLED(MovementType::TravelMode)
        .AndThen(SetPose({
            ...
            return !this->m_wrist->GetIsMovingToPosition();
        })
        // Move extension to 0
        .AndThen(SetPose({
            .axis = m_extension,
            ...
            return !this->m_extension->GetIsMovingToPosition();
        })
        // Move arm to 0
        .AndThen(SetPose({
            .axis = m_rotateArm,
            ...
            return !this->m_rotateArm->GetIsMovingToPosition();
        })
        .AndThen(SetMovementLED(MovementType::None));
}
...
frc2::CommandPtr CompleteArmSubsystem::SetPose(ArmAxisPose pose) {
    return frc2::InstantCommand(
        [pose] (){
            pose.axis->MoveToPosition(pose.position);
        }
    ).ToPtr();
